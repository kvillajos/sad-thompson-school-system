...
export function renderAcademicStudents() {
  ...
  $('academic-table').innerHTML = students.map(student => {
    ...
    return `<tr><td>${escapeHtml(student.lrn_number || student.student_id)}</td><td>${escapeHtml(`${student.first_name||''} ${student.last_name||''}`)}</td><td>${escapeHtml(gradeLabel(student.grade_level))}</td><td>${summary.count}</td><td>${escapeHtml(summary.latest || '-')}</td><td>${escapeHtml(state.studentSections.get(String(student.student_id)) || 'No Section')}</td><td><button class="admin-view" data-view-academic="${student.student_id}">View History</button> <button class="admin-view" data-print-academic="${student.student_id}">Print Card</button></td></tr>`
