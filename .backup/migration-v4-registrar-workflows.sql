...
begin
  select username into editor from users where lower(email) = lower(auth.jwt() ->> 'email') limit 1;
  editor := coalesce(editor, 'registrar');
  select * into a from admission_applications where id = p_application_id for update;
  if not found then raise exception 'Application not found'; end if;
  if a.editing_by is distinct from editor
...
begin
  select * into a from admission_applications where id = p_application_id for update;
  if not found then raise exception 'Application not found'; end if;
  if a.editing_token is distinct from p_token then
  ...
  next_status := coalesce(nullif(trim(merged->>'status'), ''), a.status::text);
