...
tabs.forEach((tab) => {
  tab.addEventListener('click', () => {
    ...
    console.log('Clicked tab:', panelId)
    console.log('Target panel:', targetPanel)

    if (!targetPanel) {
      console.error(`Panel with id="${panelId}" was not found.`)
      toast(`Page "${panelId}" could not be found.`, 'error')
      return
    }
...
async function updateApplicationStatus() {
  ...
  const { data, error } = await supabase
    .from('sections')
    .select('section_id,section_name,grade_level,capacity')
    .order('grade_level')
    .order('section_name')

  console.log('Sections returned by Supabase:', data)
  console.log('Sections error:', error)

  if (error) {
    return toast(`Sections error: ${error.message}`, 'error')
  }
...
async function openPlacement(sectionId = '') {
  ...
  if (error) {
    return toast(
      `Could not load placement students: ${error.message}`,
      'error'
    )
  }
  ...
  if (!state.students.length) {
    return toast(
      'No students are available for placement. Check the students table and RLS policy.',
      'error'
    )
  }

  // Student dropdown
  $('placement-student').innerHTML = state.students
    .map(
      s => `
        <option value="${s.student_id}">
          ${escapeHtml(s.lrn_number || '')} -
          ${escapeHtml(s.first_name)} ${escapeHtml(s.last_name)}
          (${escapeHtml(gradeLabel(s.grade_level))})
        </option>
      `
    )
    .join('')
  ...
  const grades = [...new Set(
    state.sections.map(section => Number(section.grade_level))
  )]

  $('placement-grade').innerHTML = grades
    .map(
      grade => `
        <option value="${grade}">
  ...
  function updatePlacementOptions() {
    const studentId = $('placement-student').value
    ...
    const studentGrade = Number(student.grade_level)

    // Automatically show student's grade
    $('placement-grade').value = String(studentGrade)

    // Find sections for this grade
    const matchingSections = state.sections.filter(
      section => Number(section.grade_level) === studentGrade
    )

    if (!matchingSections.length) {
      $('placement-section').innerHTML =
        '<option value="">No section available for this grade</option>'
      return
    }

    $('placement-section').innerHTML = matchingSections
      .map(
        section => `
          <option value="${section.section_id}">
            ${escapeHtml(section.section_name)}
            — ${escapeHtml(gradeLabel(section.grade_level))}
    ...
    if (
      sectionId &&
      matchingSections.some(
        section => String(section.section_id) === String(sectionId)
      )
    ) {
      $('placement-section').value = String(sectionId)
    }
  }

  // Run immediately for first student
  updatePlacementOptions()

  // Run whenever student changes
  $('placement-student').onchange = updatePlacementOptions
...
async function loadStudents() {
  ...
  if (error) {
    return toast(`Could not load students: ${error.message}`, 'error')
  }
  ...
  const html = state.students.map(s => `
    <option value="${s.student_id}">
      ${escapeHtml(s.lrn_number || s.student_id)} —
      ${escapeHtml(s.first_name || '')}
      ${escapeHtml(s.last_name || '')}
    </option>
  `).join('')

  $('academic-student').innerHTML = html
  $('transcript-student').innerHTML = html
  $('shift-student').innerHTML = html
